...
to make queries against both IPs. Note that the containers are _not_ using port 80 on
...
Deployment to recreate them. This time around the Service exists _before_ the
...
- Each container has access to the keys through a volume mounted at `/etc/nginx/ssl`.
  This is set up _before_ the nginx server is started.
