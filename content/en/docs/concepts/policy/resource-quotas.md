...
If the `operator` is `Exists` or `DoesNotExist`, the `values` field must _NOT_ be
specified.
...
expressed in absolute units. So, if you add nodes to your cluster, this does _not_
